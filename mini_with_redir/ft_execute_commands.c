...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/18 10:37:49 by iboiraza          #+#    #+#             */
/*   Updated: 2025/03/18 10:38:00 by iboiraza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	init_signals(&((*data).signal));
	if (data->nbr_nodes == 1)
	{
        ...
        if (execute_builtin(data->nodes[0]->full_cmd, data->envp, &data))
...
//			execute_builtin(data->nodes[0]->full_cmd[0], data, 0);
		else if (!is_builtin(data-	ft_exit(data);>nodes[0]->full_cmd[0])
			&& data->nodes[0]->is_set)
			execute_simple_command(data, data->nodes[0], pid);
//    printf("count->%i\n", data->ft_count_pipes);
//    printf("nbr_nodes->%i\n",data->nbr_nodes);
