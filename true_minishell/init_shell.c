...
/*   Updated: 2025/04/10 19:42:35 by iksaiz-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_env_name(char *argv, int x)
{
	...
	while (var_name[0][i])
	{
		if (!ft_isalpha(var_name[0][i]) && var_name[0][i] != '_')
		{
...
void	asign_env_value(char *argv, t_prompt **data)
{
	...
	if (!var)
		return ;
	var->envp = ft_strdup(argv); // Si no existe una variable con ese nombre, se crea una nueva y se asigna el valor de argv
	var->next = NULL;
	free(var_name);
